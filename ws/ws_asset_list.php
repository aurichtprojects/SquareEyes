...
try {
	$format = 'json';
	$sql = sanitizeSQL("SELECT a.id as id,a.label as text,(select case(select count(*) from nz.potential_distribution_current pdc where pdc.asset_id=a.id) when 0 then 'f' else 't' end) as pd_flag FROM nz.asset a ORDER BY label");
	$pgconn = pgConnection();
